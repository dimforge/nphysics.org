...
[iterators](#interaction-iterators) provided by the collider world.

## Sensors
Sensors are a special type of [colliders](/rigid_body_simulations_with_contacts/#colliders) that don't generate any contact.
Like reglar colliders, they have to be added to the world, are given a geometric shape, and are attached to a body part
(or the `BodyHandle::ground()`). They are commonly used for detecting proximity, e.g., to detect when a player enters a
...
Since sensors are just special kinds of colliders, they can be removed from the world using `world.remove_colliders(...)`
and retrieved with `world.collider(...)`. As presented in the next [section](/event_handling_and_sensors/#event-handling)
...
world at each timestep. It is possible to iterate through all the events generated by the last call to `world.step()`:
...
!!! Warning
    ...
    before the last call to `world.step()` are no longer retrievable.

!!! Note
    Contact and proximity events identify the involved colliders by their handle. It is possible to retrieve the handle of
    the body part a collider is attached to with `world.collider_body_handle(collider_handle)`. This will return
    `BodyHandle::ground()` if the collider is attached to the ground.
...
```rust
let collider_world = world.collider_world();
if let Some(collider1, collider2, algorithm, manifold) = collider_world.contact_pair(handle1, handle2) {
   // ...
}
```
...
one using the `ColliderWorld` stored by the physics `World`.
